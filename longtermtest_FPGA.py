...
from longtermtest_serialfcns import readFPGA, ser_write, response_check
...
def reset_PIC_FPGA(pic_ser, pic_ser1, pic_ser2, FPGA_ser):
    ...
    time.sleep(0.5)   
    FPGA_ser.open() 
    return(pic_ser, pic_ser1, pic_ser2, FPGA_ser)

def config_FPGA(FPGA_ser, freq0, freq1, freq2, freq3, freq4, date_str, time_bool ):
    ...
    # Checking the RAW_DATA define
    time_CCSDS = True
    if time_bool == 0:
        time_CCSDS = False
    file_path = './LongTerm_data'+'/'+'FPGA-' + FPGA_rev + '_iter' + date_str 
    vals = readFPGA(FPGA_ser, freq0, freq1, freq2, freq3, freq4, time_CCSDS, readcon=readcon,num_read=1,outpath=file_path, byte_type=2)
    print("Packets saved")

def buffering_FPGA(pic_ser, pic_ser1, pic_ser2, FPGA_ser):
