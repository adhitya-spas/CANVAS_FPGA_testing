...
def readFPGA(ser, freq0, freq1, freq2, freq3, freq4,time_CCSDS, num_read = 1, readcon = 'none', outpath = 'HW-output/default-file', byte_type = 2):
    #define data modes
    tx_packet_gen = b'\x02'
    rotation = b'\x03'
    fft_result = b'\x04'
    ...
    CCSDS_packets = b'\x00'# Back to b'\x00' from b'\x08' # Changed again: b'\x00' # Previously '\x08'
    ...
    #read in payload 
        if test_mode==tx_packet_gen:
            ...
            date_time = outpath[-15:]
            outpath=outpath
            name = outpath+ 'CCSDS_pkt' + date_time + '_' 
