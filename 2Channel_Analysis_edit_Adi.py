...
show_plots = False
include_transmitter = False #True
...
#Base File paths
pypy_base = './Code-output/'+amp #Code-output\high-high_5deg_03khz
FPGA_base = "./HW-output/FPGA-"+rev+amp
...
for f in range(freqs):
    ...
    for ph in range(phases):
        ...
        print(test_conditions) # for debugging

        fpga_file_im = FPGA_base+test_conditions+'_xpec_im_avg_hex.txt'
        fpga_file_re = FPGA_base+test_conditions+'_xpec_re_avg_hex.txt'

        #pypy_file_avg_im = pypy_base+test_conditions+'i_avg_hex.txt'
        #pypy_file_avg_re = pypy_base+test_conditions+'r_avg_hex.txt'

        pypy_file_comp_re = pypy_base+test_conditions+'/'+amp+test_conditions+'_channel_i_cmprs_hex.txt'
        pypy_file_comp_im = pypy_base+test_conditions+'/'+amp+test_conditions+'_channel_r_cmprs_hex.txt'
