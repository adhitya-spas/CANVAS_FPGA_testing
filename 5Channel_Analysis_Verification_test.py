...
if fromFile:
    ...
    channels1_td = read_FPGA_input(file0,signed=True,show_plots=False)
    channels2_td = read_FPGA_input(file0,signed=True,show_plots=False)
    channels3_td = read_FPGA_input(file0,signed=True,show_plots=False)
    channels4_td = read_FPGA_input(file0,signed=True,show_plots=False)
...
# #     now = datetime.now()
# #     date_time = now.strftime("_%m%d%Y_%H%M%S")
# #     vals = readFPGA(FPGA_ser,readcon=readcon,num_read=num,outpath=out_folder+'/5-ch'+'/Looped_twice_all'+'/'+amp+'FPGA-' + FPGA_rev + amp + phase + f + '_iter' + str(iterate) + date_time)
