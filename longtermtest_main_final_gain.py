...
switch_time = 30        # seconds (prev 10 sec)
...
ef_amp_list = np.arange(start =hi_amp_ef*0.00, stop = hi_amp_ef*1.5, step = hi_amp_ef*0.01).tolist()
bf_amp_list = np.arange(start =hi_amp_bf*0.00, stop = hi_amp_bf*1.5, step = hi_amp_bf*0.01).tolist()

# Trigger for first time to set phase and freq only once
...
while(True):
    ...
    if FIXED_AMP == 2:
        if sync_mark == 0:
            ...
            amp1=bf_amp_list[counter_a]
            amp2=ef_amp_list[counter_a]
            amp3=bf_amp_list[counter_a]
            amp4=ef_amp_list[counter_a]
    ...
    b_in_amp1 = amp1/10 # previous 10 | 50dB
    b_in_amp2 = amp2/708 # previous 10 | 50dB
    b_in_amp3 = amp3/10  # 20 dB
    b_in_amp4 = amp4/708  # 20 dB
    b_in_amp5 = amp5/10  # 20 dB

    sr_amp1 = amp1/(hi_amp_bf)
    sr_amp2 = amp2/(hi_amp_ef)
    sr_amp3 = amp3/(hi_amp_bf)
    sr_amp4 = amp4/(hi_amp_ef)
    sr_amp5 = amp5/(hi_amp_bf)
    ...
    time.sleep(switch_time-2)     # switch_time can be set up in the MACRO SECTION in Line 53
