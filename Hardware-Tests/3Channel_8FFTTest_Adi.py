...
while iterate < 3:
    ...
    SCM_x00 = twos_complement_to_hex(1)
    SCM_x01 = twos_complement_to_hex(0)
    SCM_x02 = twos_complement_to_hex(0)
    SCM_y10 = twos_complement_to_hex(0)
    SCM_y11 = twos_complement_to_hex(1)
    SCM_y12 = twos_complement_to_hex(0)
    SCM_z20 = twos_complement_to_hex(0)
    SCM_z21 = twos_complement_to_hex(0)
    SCM_z22 = twos_complement_to_hex(1)
    SCM_xoff = twos_complement_to_hex(0)
    SCM_yoff = twos_complement_to_hex(0)
    SCM_zoff = twos_complement_to_hex(0)

    # SCM_x00 = r'\x'+SCM_x00[:2]+r'\x'+SCM_x00[2:]
    # SCM_x00 = (SCM_x00[1:4] + SCM_x00[5:]).encode()
    # SCM_x01 = SCM_x01.encode()
    # SCM_x02 = SCM_x02.encode()
    # SCM_y10 = SCM_y10.encode()
    # SCM_y11 = SCM_y11.encode()
    # SCM_y12 = SCM_y12.encode()
    # SCM_z20 = SCM_z20.encode()
    # SCM_z21 = SCM_z21.encode()
    # SCM_z22 = SCM_z22.encode()
    # SCM_xoff = SCM_xoff.encode()
    # SCM_yoff = SCM_yoff.encode()
    # SCM_zoff = SCM_zoff.encode()
    ...
    ####ser_write(FPGA_ser,Sync_Pat+OpcodeSetMatrix+SCM_x01+SCM_x00+SCM_y10+SCM_x02+SCM_y12+SCM_y11+SCM_z21+SCM_z20+SCM_xoff+SCM_z22+SCM_zoff+SCM_yoff,False)
    print('FPGA Configured')
    time.sleep(0.5)
    ser_write(FPGA_ser,Sync_Pat+Config+spec_core+testmode,False)
    print('Set Test Mode')
    ...
    vals = readFPGA(FPGA_ser,readcon=readcon,num_read=num,outpath=out_folder+'/FPGA-' + FPGA_rev + amp + phase + f + '_' + str(iterate))
